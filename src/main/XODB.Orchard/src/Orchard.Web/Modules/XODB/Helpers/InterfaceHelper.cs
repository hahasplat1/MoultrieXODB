            ...
            public static Type Build(Type interfaceType)
            {
                try
                {
                    ...
                    if (_interfaceTypeCache.TryGetValue(interfaceType.FullName, out type))
                        return type;
                    AssemblyName assemblyName = new AssemblyName("XODB.Helpers.Interfaces");
                    AssemblyBuilder assemBuilder = Thread.GetDomain().DefineDynamicAssembly(assemblyName, AssemblyBuilderAccess.RunAndSave);
                    ModuleBuilder moduleBuilder = assemBuilder.DefineDynamicModule("XODBDataBuilderModule", true);
                    TypeBuilder typeBuilder = moduleBuilder.DefineType("XODB.Helpers.Interfaces." + interfaceType.Name, TypeAttributes.Class | TypeAttributes.Public);
