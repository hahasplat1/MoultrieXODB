...
namespace XODB.Import.ImportUtils
{
    public class AssayImport
    {
                                    ...
                                    string connectionString, Guid XODBProjectID, bool checkForDuplicates, bool doImportOverwrite)
        {
            ...
            XODBImportAssayEntities entityObj = new XODBImportAssayEntities();

            entityObj.Configuration.AutoDetectChangesEnabled = false;
            Guid agGuid = Guid.NewGuid();
            X_AssayGroup ag = new X_AssayGroup();
           ...
           // set up the assay test columns - one of these for each test type
            ...
            Dictionary<Guid, X_AssayGroupTest> assayGroups = new Dictionary<Guid, X_AssayGroupTest>();
            ...
            foreach (KeyValuePair<ColumnMap, Guid> kvp in resultsColumns) {
                ...
                Guid g = kvp.Value;
                X_AssayGroupTest xt = new X_AssayGroupTest();
                ...
                entityObj.X_AssayGroupTest.Add(xt);
            ...
            try
            {
                ...
                assayQueries.SetEntityObject(entityObj);
                    ...
                    while ((line = sr.ReadLine()) != null)
                    {
                        ...
                        if (ct >= importMap.dataStartLine)
                        {
                            ...
                            else
                            {
                                ...
                                List<X_Sample> duplicateList = null;
                                ...
                                foreach (KeyValuePair<ColumnMap, Guid> kvp in resultsColumns)
                                {
                                    ...
                                    X_AssayGroupTestResult testResult = new X_AssayGroupTestResult();
                                    /*bool assayResFound = false;
                                    
                                    if (isDuplicateInterval)
                                    {
                                       List<X_AssayGroupTestResult> testResults = assayQueries.GetDuplicateResult(xs.SampleID, cm.sourceColumnName);
                                   ...
                                   // {
                                    ...
                                    //{                                       
                                        entityObj.X_AssayGroupTestResult.Add(testResult);
                                        assayAdded = true;
        ...
        private X_AssayGroupTest FindExistingAssayGroupTest(string p)
        {
            XODBImportAssayEntities e = new XODBImportAssayEntities();
            X_AssayGroupTest resAssGroup = null;
            IQueryable<X_AssayGroupTest> res = e.X_AssayGroupTest.Where(c => c.AssayTestName.Trim().Equals(p.Trim()));
            foreach (X_AssayGroupTest xx in res)
            {
                resAssGroup = xx;
            }
            return resAssGroup;
        ...
        {
            Guid pid = new Guid();
            X_Parameter xp = new X_Parameter();

            XODBImportEntities e = new XODBImportEntities();
            bool found = false;
            IQueryable<X_Parameter> res = e.X_Parameter.Where(c => c.ParameterType.Equals("AssayTypeName") && c.ParameterName.Equals(pName));
            foreach (X_Parameter xx in res)
            {
                found = true;
                pid = xx.ParameterID;
                break;
            ...
            if (!found) {
                X_Parameter pp = new X_Parameter();
                pid = Guid.NewGuid();
                pp.ParameterID = pid;
